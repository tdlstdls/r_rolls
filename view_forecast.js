...
function generateFastForecast(initialSeed, columnConfigs) {
    ...
    // 修正: ボタンの右側に注釈を追加
    summaryHtml += `
        <div style="margin-bottom: 15px; text-align: left; display: flex; align-items: center; gap: 10px;">
            <button onclick="toggleAllFindVisibility()" class="secondary" style="font-size: 11px; padding: 4px 8px;">${toggleBtnText}</button>
            <span style="font-size: 0.85em; color: #666;">※ 個別の表示/非表示の選択はガチャマスター情報からも出来ます</span>
        </div>
    `;
    // -------------------------------------------------------------------------
    // 各ガチャ列ごとのターゲット表示
